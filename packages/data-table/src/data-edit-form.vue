    ...
    <ii-form :model="form" v-if="dataReady" :rules="rules" ref="form" :label-width="formDefine.labelWidth">
      <ii-row v-for="(row, index) in fromprop" :key="'formRow' + index">
          <ii-col v-for="item in row" v-if="!item.hidden" :span="item.span" :key="item.model">
            <ii-form-item :label='item.getdefinelabel ? item.getdefinelabel($data) : $t(item.label)' v-if="String(item.control).toLowerCase().indexOf('select') === -1 && String(item.control).toLowerCase().indexOf('multi-select') === -1 && String(item.control).toLowerCase().indexOf('selectval') === -1 && String(item.control).toLowerCase().indexOf('cascader') === -1 && String(item.control).toLowerCase().indexOf('cascaderval') === -1 && String(item.control).toLowerCase().indexOf('upload') === -1 && String(item.control).toLowerCase().indexOf('slot') === -1" :prop="item.model">
              <ii-row v-if="item.control === 'input'">
                <ii-col :span="item.unit ? 23 :24">
                  <ii-input v-if="item.model.indexOf('.') === -1" :disabled="(editdata && item.dismodify) || item.disabled" v-model="form[item.model]"></ii-input>
                  <ii-input v-else :disabled="(editdata && item.dismodify) || item.disabled" v-model="form[item.model.split('.')[0]][item.model.split('.')[1]]"></ii-input>
                </ii-col>
                <ii-col :span="1" justify="end" v-if="item.unit">
                  <span>{{item.unit($data)}}</span>
                </ii-col>
              </ii-row>
              <ii-input :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :disabled="(editdata && item.dismodify) || item.disabled" type="textarea" :rows="4" v-if="item.control === 'textarea' && item.model.indexOf('.') === -1" v-model="form[item.model]"></ii-input>
              <ii-input :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :disabled="(editdata && item.dismodify) || item.disabled" type="textarea" :rows="4" v-if="item.control === 'textarea' && item.model.indexOf('.') > 0" v-model="form[item.model.split('.')[0]][item.model.split('.')[1]]"></ii-input>
            ...
            <ii-form-item :label='$t(item.label)' v-if="item.control === 'select'" :prop="item.model">
              <ii-select :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :ref="item.model" :disabled="(editdata && item.dismodify) || item.disabled" v-model="form[item.model.split('.')[0]][item.model.split('.')[1]]" :valueObj.sync="form[item.model.split('.')[0]]" :initOptions="item.control_options" v-if="item.control === 'select'" ></ii-select>
           </ii-form-item>
            <ii-form-item :label='$t(item.label)' v-if="item.control === 'selectval'" :prop="item.model">
              <ii-select :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :ref="item.model" :disabled="(editdata && item.dismodify) || item.disabled" v-model="form[item.model]" :valueObj.sync="form[item.model.split('.')[0]]" :initOptions="item.control_options" v-if="item.control === 'selectval'" ></ii-select>
           </ii-form-item>
            ...
            <ii-form-item :label='$t(item.label)' v-if="item.control ==='cascader'" :prop="item.model">
            ...
            <ii-form-item :label='$t(item.label)' v-if="item.control ==='cascaderval'" :prop="item.model">
              <ii-cascader :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :disabled="(editdata && item.dismodify) || item.disabled" v-model="form[item.model]" :valueObj.sync="form[item.control_options.value_path[item.control_options.value_path.length - 1]]" :initOptions="item.control_options" v-if="item.control === 'cascaderval'"></ii-cascader>
            </ii-form-item>
            <ii-form-item :label='$t(item.label)' v-if="item.control ==='upload'">
              <ii-upload :placeholder='item.placeholder ? $t(item.placeholder) : undefined' :disabled="(editdata && item.dismodify) || item.disabled" :file-list.sync="form[item.model]" :initOptions="item.control_options" v-if="item.control === 'upload'"></ii-upload>
            </ii-form-item>
            <ii-form-item :label='$t(item.label)' v-if="item.control ==='slot'" :prop="item.model">
      ...
      if (editData) {
        ...
        _.each(this.formDefine.prop, (row) => {
          _.each(row, (item) => {
            ...
            if (item.control === 'select') {
              item_models = item.model.split('.')
              this.form[item_models[0]][item_models[1]] = editData[item_models[0]] ? (editData[item_models[0]][item_models[1]] === -1 ? null : editData[item_models[0]][item_models[1]]) : null
            } else if (item.control === 'multi-select') {
              ...
              item_models = item.model.split('.')
              this.form[item_models[0]][item_models[1]] = editData[item_models[0]] ? editData : null
            } else if (item.control === 'cascaderval') {
              this.form[item.model] = editData[item.model] ? editData : null
            } else if (item.control === 'datetime') {
              if (item.model.indexOf('.') > 0) {
                item_models = item.model.split('.')
                this.form[item_models[0]][item_models[1]] = editData[item_models[0]] ? (editData[item_models[0]][item_models[1]] ? moment(editData[item_models[0]][item_models[1]]).format('YYYY-MM-DD HH:mm:ss') : null) : null
              } else {
                this.form[item.model] = editData[item.model] ? moment(editData[item.model]).format('YYYY-MM-DD HH:mm:ss') : null
              }
            } else {
              if (item.model.indexOf('.') > 0) {
                item_models = item.model.split('.')
                this.form[item_models[0]][item_models[1]] = item['displayformat'] ? item['displayformat'](editData) : editData[item_models[0]][item_models[1]]
              } else {
                this.form[item.model] = item['displayformat'] ? item['displayformat'](editData) : editData[item.model]
              }
            }
        ...
        this.form[this.formDefine.primary_column] = editData[this.formDefine.primary_column]
        this.title = this.formDefine.module_name + '-' + this.$t('dialog.edit_title')
      } else {
        this.setinitvalue()
        this.title = this.formDefine.module_name + '-' + this.$t('dialog.add_title')
      }
      // _.each(this.formDefine.prop, (row) => {
      //   _.each(row, (item) => {
    ...
    async handleSave () {
      ...
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          let data_to_save = _.cloneDeep(this.form)
          if (this.before_save) {
            before_save = await this.before_save(this.editdata, data_to_save, this.$data)
          }
          ...
          _.each(this.formDefine.prop, (row) => {
            _.each(row, (item) => {
              if (_.isFunction(item['inputconvert'])) data_to_save[item['key']] = item['inputconvert'](data_to_save[item['key']])
              data_to_save[item.model] = data_to_save[item.model] ? (_.isObject(data_to_save[item.model]) ? data_to_save[item.model] : (_.isString(data_to_save[item.model]) ? data_to_save[item.model].trim() : data_to_save[item.model])) : null
            })
          })
          if (!this.editdata && _.hasIn(this.formDefine, 'initparams')) {
            data_to_save[this.formDefine['initparams'].key] = this.formDefine['initparams'].value
          }
          if (this.editdata) {
            _.each(this.tableData, (row, index) => {
              if (row[this.formDefine.primary_column] === data_to_save[this.formDefine.primary_column]) {
                this.$set(this.tableData, index, data_to_save)
              }
            })
            ...
            if (!data_to_save[this.formDefine.primary_column]) {
              this.$set(data_to_save, this.formDefine.primary_column, this.tableData ? (_.maxBy(this.tableData, this.formDefine.primary_column) ? (_.maxBy(this.tableData, this.formDefine.primary_column)[this.formDefine.primary_column] || 0) : 0) + 1 : 0)
            }
            this.tableData.push(data_to_save)
            this.saved_data = data_to_save
            this.haveSaved = true
            let _this = this
